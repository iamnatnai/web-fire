...
foreach ($dataByFTank as $fTank => $entries) {
    ...
    array_push($headers, 'หมายเหตุ', 'ผู้ประเมิน', 'รูปภาพ');
    ...
    foreach ($entries as $index => $entry) {
        ...
        if (!empty($entry['image'])) {
            $imagePath = 'evaluation_image/' . $entry['image'];
            if (file_exists($imagePath)) {
                $rowData[] = $imagePath; // Include image path
            } else {
                $rowData[] = 'ไม่มีการแนบรูป'; // No image attached
            }
        } else {
            $rowData[] = 'ไม่มีการแนบรูป'; // No image attached
        }
